...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderSummary(data) {
        ...
        const lunas = data.filter(d => d.lunas).length;
        const belum = totalPelapak - lunas;
        const totalNominal = data.reduce((sum, d) => sum + Number(d.total), 0);

        summary.innerHTML = `
      <div class="summary-box">
        <span>Total Pelapak</span>
        <strong>${totalPelapak}</strong>
      </div>
      <div class="summary-box">
        <span>Lunas</span>
        <strong>${lunas}</strong>
